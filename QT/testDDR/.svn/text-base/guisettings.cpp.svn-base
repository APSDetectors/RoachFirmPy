...
void guiSettings::setupPython(void)
{
    ...
    char cmd[256];
    void *ptr;
    FILE *cmdfile = fopen("roachsetup.py","r");
...
void guiSettings::roachWriteInt(QString regname, int val)
{
    ...
    cmd = QString("roach2.write_int('%1',%2)").arg(regname).arg((u_int32_t)val);
...
int guiSettings::roachReadInt(QString regname)
{
    ...
    cmd = QString("valx = roach2.read_int('%1')").arg(regname);
    writeLine(cmd);
...
QString guiSettings::roachRead(QString regname,int count,char *data)
{
    QString cmd;
