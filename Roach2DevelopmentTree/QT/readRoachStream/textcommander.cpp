...
bool textCommander::openInputPipe(void)
{
    if ( !is_in_pipe_open)
    {
        ...
        if (in_pipe!=0)
        {
               ...
               fprintf(stderr,"Input pipe opend %s\n",in_pipe_name.toStdString().c_str());
        }
        else
        {
            is_in_pipe_open=false;
...
bool textCommander::openOutputPipe(void)
{
    if ( !is_out_pipe_open)
    {
        ...
        if (out_pipe!=0)
        {
               ...
               fprintf(stderr,"Output pipe opend %s\n",out_pipe_name.toStdString().c_str());
        }
        else
        {
            is_out_pipe_open=false;
...
bool textCommander::waitNextCommand(void)
{
    ...
    while(is_waiting && is_in_pipe_open)
    {
        ...
        if (args.length()>=4)
        {
            ...
            else
            {
                if (is_out_pipe_open)
                {
                    fprintf(stderr,"Resending Cmd: %s\n",line);
                    fputs(line,out_pipe);
                    fflush(out_pipe);
                }
    ...
    fprintf(stderr,"NOT Waiting for Command\n");
    fflush(stdout);
}
...
bool textCommander::sendCommand(QString &objname, QString &slotname,int numargs,QStringList &args)
{
